...
### ~~Priority 86 — Dependency Graph: bottleneck fix, always-on View Options, richer/simpler links, auto-edge info panel~~ ✓ Done `[group: graph-links]`
A run of small, real-usage-driven fixes to Graph Hub across one session, each triggered by a specific user report rather than a planned batch.

- **Bottleneck detection, twice-corrected** — first report: "highlighting too many nodes... tags as bottleneck seems not correct." Root cause was `computeBottlenecks()` reusing the broad `_impactAdjacency()` (includes auto/tag edges and undirected `relates`), so any well-tagged item looked like a false-positive single point of failure. Added a narrower `_bottleneckAdjacency()` restricted to genuine causal links only (`blocks`/`depends-on`, same convention Critical Path uses), excluding `_auto` edges and `relates` entirely. Second report after that fix: "no bottlenecks at all" — `BOTTLENECK_MIN_IMPACT` lowered from 3 to 2.
- **View Options always-visible, collapsed by default** — was a button-toggled panel; changed to permanently rendered with a `.collapsed` class hiding the body, removing the extra click to check current state. This introduced a regression: right-click "Add link from here…" stopped responding. Root-caused via a real headless-browser render (`elementFromPoint` at the click coordinates) to the always-visible panel sitting at `top:20px;left:20px` — directly on top of the canvas, silently absorbing clicks on any node underneath. Fixed by repositioning to `bottom:60px;left:20px`, stacked above the existing `.controls` toolbar row instead of creating a new dead zone.
- **Drag-to-quick-link was "escaping"** — dragging a node toward another to trigger the link prompt failed because live physics kept repelling the target node in real time during the drag. `dragStart`/`dragEnd` handlers now pause/resume physics for the duration of a node drag. Also replaced `getNearbyNode()`'s sparse 8-point pixel-offset grid with a proper nearest-node-within-radius search via `network.getPositions()`/`DOMtoCanvas()` — more forgiving and correctly scales with zoom.
- **Links gained an optional note at creation time** — "+ New Link" modal gained a "Why are these connected?" textarea (`#nl-note`); previously the only way to attach a reason was creating the link, then separately reopening the edge panel afterward, an easy step to skip that left most links meaningless by default. `HubLinks.addLink()` now accepts and persists `opts.note`.
- **Redundant right-click menu item removed** — "Add link from here…" duplicated the node panel's existing "+ Link" button one click away; deleted from the context menu.
- **Relationship types consolidated 4 → 3** — "Blocks" and "Leads to" were already functionally identical everywhere in the graph engine (Critical Path, Bottleneck, and Impact Analysis all treated them the same in every direction check), so offering both was pure decision fatigue nudging people toward the effortless "Relates" default. Merged into one "→ Blocks / leads to" option. Initially only the creation dropdown was merged (edit dropdown kept all 4, deliberately, so existing `'leads-to'` data wouldn't get silently reassigned on open) — completed properly on a follow-up user report ("still 4 in the edit dropdown") by adding a one-time idempotent migration in `hub-links.js` (`_migrateLegacyRelTypes()`, runs on load, only writes if something actually changed) that rewrites any stored `relType:'leads-to'` to `'blocks'`. With no legacy data left to protect, removed `'leads-to'` from the edit dropdown, `EDGE_TYPE_CONFIG`, and the two tooltips still listing it, and simplified the now-dead `'blocks' || 'leads-to'` OR-checks in both adjacency builders down to just `'blocks'`.
- **Auto-generated edges now show a read-only info panel instead of doing nothing** — final report: "clicking a link... doesn't show up anymore." Extensive investigation (line-by-line diff review of the consolidation commit, and a real-browser click test replaying the exact migration path) found the manual-link panel was never actually broken — user follow-up clarified the real gap: clicking a *tag* or *risk→project* edge (an `_auto: true` structural edge, not a `hub-links-v1` record) has always silently done nothing, which reads as broken once you go looking for it. `selectEdge` now branches on edge kind: `_linkId` → existing editable panel; `_auto` → new `showAutoEdgePanel()`, reusing the same `#edge-panel` DOM (split into an `#edge-editable` block and a new read-only `#edge-auto-info` block, toggled per open) showing the two endpoint labels and the edge's existing descriptive `title` (e.g. "tagged: BIM", "risk → project") plus a one-line explanation that it's derived data, not an editable link — edit the source item instead.

**Key decisions:**
- **Decision:** Bottleneck's adjacency stays intentionally narrower than Impact Analysis's, permanently — not a bug to reconcile between the two. **Why:** "is this a single point of failure" and "why does this exist / what does this affect" are different questions; a shared tag or project is real context for the second but not a dependency for the first. **Confidence:** high.
- **Decision:** Fix the View Options click-blocking bug by repositioning the panel, not by adding pointer-events logic to let clicks pass through. **Why:** the panel needs its own controls to be clickable; a positional fix that gives it a zone with no nodes underneath is simpler and has no edge cases. **Confidence:** high.
- **Decision:** Auto edges get a read-only info panel, not an editable one and not silence. **Why:** they're derived from other tools' data (a tag, a `projectId`) with no `hub-links-v1` record to save a relType/note into — presenting fake edit controls would mislead; staying fully silent is what looked like a regression in the first place. A short explanation of *why* it's not editable, plus what to edit instead, closes the loop without adding a second data-storage path for the same relationship. **Confidence:** high.
- **Decision:** The 'leads-to'→'blocks' migration runs unconditionally on `hub-links.js` load (module-level IIFE), not behind a feature flag or one-time-run guard keyed elsewhere. **Why:** it's naturally idempotent (checks `relType === 'leads-to'` and only writes if something changed), so re-running it forever costs nothing and needs no separate "have I migrated" bookkeeping. **Confidence:** high.
