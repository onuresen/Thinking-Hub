...
`hub-session-v1`, `project-hub-v1`, `schedule-v1`, `decision-hub-v1`, `kmqt_current_v2` ⚠ orphaned (KMQT Board deleted P88, data retained), `canvas-v1`, `hub-links-v1`, `ideaswipe_history_v6`, `hub-cloud-config-v1`, `th-theme`, `tutorial-seen-v1`, `quick-tour-seen-v1`, `focus-hub-v1`, `log-hub-v1`, `retro-hub-v1`, `assumptions-hub-v1`, `review-hub-v1`, `matrix-hub-v1`, `meetings-hub-v1`, `goals-hub-v1`, `learning-hub-v1`, `stakeholder-hub-v1`, `risk-hub-v1`, `argument-hub-v1` ⚠ orphaned (Argument Hub deleted P120, data retained), `scrum-hub-v1` ⚠ orphaned (tool deleted P50, data retained), `hub-activity-v1`, `hub-settings-v1`, `tool-portfolio-v1`, `reflection-hub-v1` ⚠ orphaned (Reflection Board deleted P120, data retained), `hub-warroom-v1` ⚠ orphaned (War Room deleted P50, data retained), `hub-resurface-v1` (ephemeral — Resurface dismiss-state, P51; excluded from backup/sync like other UI-state keys), `hub-tags-v1` (central tag/topic registry, P57), `hub-last-backup-v1` (ephemeral — last-full-backup + nudge timestamps, P80; excluded from backup/sync), `capture-hub-v1` (Capture Hub inbox), `people-hub-v1` (People Hub roster/org tree), `machi-milestones-v1` (ephemeral — Machi milestone dedupe/baseline state; deliberately excluded from backup/sync per Codex P81C decision), `rb-migration-done-v1` / `rb-reacted-v1` ⚠ orphaned (ephemeral — Reflection Board migration flag + reaction state; tool deleted P120), `ai-drawer-pos-v1` (ephemeral — AI drawer position), `hub-briefing-v1` (ephemeral — cached daily AI morning briefing `{date, text}`, P86; excluded from backup/sync), `hub-vault-bridge-v1` (Vault Bridge state — daily-notes folder, cached unrecorded days, and the accepted/ignored dedupe map; **backed up + synced**, because losing `seen` would re-propose already-imported decisions as duplicates)

## Runtime dependencies
| Component | Used in | Version / source |
|-----|---------|---------|
...
**Files:** `meetings-hub.html`, `schedule.html`, `graph-hub.html`, `index.html`, `achievements-hub.html`, `CLAUDE.md`
...
### ~~Priority 79 — Project Hub: Priority Matrix reframed as Impact × Effort~~ ✓ Done `[group: project-ux]`
User feedback: the Eisenhower Urgent/Important matrix's "Delegate" and "Eliminate" quadrants "seem more like an evaluation for software" than something that fits real project/task triage. Root cause discussed first: Eisenhower's matrix was built for a manager's daily inbox (meetings, requests from others) — "Delegate" only makes sense with a team under you, which doesn't match this app's solo-strategist user (same lesson as the War Room/Scrum Board deletions, Priority 50).

**Implementation (`project-hub.html`):** Swapped to the **Action Priority Matrix** (Impact × Effort) — a real named framework, not invented, with no quadrant that assumes anyone but you. Quadrants: **Quick Win** (High Impact, Low Effort — do first), **Major Project** (High Impact, High Effort — schedule deliberately), **Fill-in** (Low Impact, Low Effort — batch when idle), **Thankless Task** (Low Impact, High Effort — avoid/cut). Axis labels changed from "Urgency →" / "Not Important · Important" to "Impact →" / "Low Effort · High Effort". Quadrant colors moved off ad-hoc hardcoded hex (`#ef4444`, `#f59e0b`, plus matching `rgba()` tints) onto the existing `--node-*`/`--border-*` green/blue/gray/red token pairs already used the same way elsewhere (Tool Portfolio chips, Capture Hub type badges) — fixes a pre-existing "no new hex" violation as a side effect of the rename, not a separate pass.

**Data migration:** `MX_LEGACY_MIGRATION` maps old→new quadrant keys (`do-now→quick-win`, `schedule→major-project`, `delegate→fill-in`, `eliminate→thankless`), applied idempotently inside `loadMx()` — only writes back to `matrix-hub-v1` if a legacy value is actually found, so existing users' triaged items land in the analogous new quadrant automatically, no data loss. `syncMxWithTasks()`'s auto-import default (previously `'schedule'`, the "important but not urgent" holding pen for freshly-synced open tasks) changed to `'fill-in'` — the least presumptuous quadrant, since an untriaged task's real impact/effort isn't known yet.

**Key decisions:**
- **Decision:** Reuse the exact same 2×2 spatial drag-drop UI, just relabel axes/quadrants/colors — not a rebuild. **Why:** the spatial triage motion (see everything at once, drag to reprioritize) was never the complaint; only the *label semantics* didn't fit. Rebuilding the interaction layer would be scope creep on a framing fix. **Confidence:** high.
- **Decision:** Legacy quadrant → new quadrant mapping is `do-now→quick-win`, `schedule→major-project`, `delegate→fill-in`, `eliminate→thankless`, not a different pairing. **Why:** preserves each old quadrant's *intent* as closely as the two frameworks allow — "urgent & important, act now" → "high impact, low effort, act now"; "important, not urgent, plan for it" → "high impact, high effort, deliberate work"; "urgent but not important, get it off your plate" → "low impact, low effort, harmless"; "not urgent, not important, drop it" → "low impact, high effort, actively avoid" (the strongest "stop doing this" quadrant in the new scheme). **Alternative:** a straight positional mapping (same grid slot, ignore intent) — rejected, would silently misfile items like "eliminate" (drop entirely) into "fill-in" (harmless busywork) rather than "thankless" (actively avoid). **Confidence:** med — reasonable but not the only defensible mapping; a user could disagree on any one pairing without it being wrong.
- **Decision:** `syncMxWithTasks()`'s auto-added-task default changed from `'schedule'` to `'fill-in'`. **Why:** the original comment already called this bucket "untriaged" — `'schedule'` (Important, Not Urgent) made an implicit importance claim about a task nobody had reviewed yet; `'fill-in'` (Low Impact, Low Effort) is the least presumptuous landing spot in the new scheme, matching the original's actual intent better than a literal analog would. **Confidence:** med.
- **Decision:** Did not touch the retired standalone `matrix-hub.html` (unreachable from the sidebar since Priority 24's "Priority Matrix → Project Hub view" consolidation). **Why:** matches this codebase's standing precedent for retired tool files (kmqt-board.html, etc.) — left as historical/dead code, not actively maintained. **Confidence:** high.
...
### ~~Priority 88 — Declutter: remove redundant per-tool export/import + retire KMQT Board~~ ✓ Done `[group: declutter]`
A two-part decluttering pass driven by the user reviewing what they actually use. Extends the P50 lesson (delete unused surfaces rather than leave them parked).

**Part 1 — Remove redundant per-tool JSON export/import.** The ⚙ Data & Backup menu already covers all export/import (Full Backup = restorable whole-app; Current Tool = per-tool export), so scattered per-tool JSON buttons were redundant — and **Project Hub's `importJSON` was a footgun**: it replaced `project-hub-v1` wholesale while links/schedule/decisions kept referencing the old state, creating dangling cross-tool references. Removed: Project Hub's Export/Import topbar buttons + `#import-file` input + `exportJSON`/`importJSON`; Reflection Board's "↓ JSON" export button + `exportJSON`. **Kept** (distinct features, not backup redundancy): all markdown/PNG "share" exports, `.ics` import, cross-tool "Import KMQT/Retro/tasks" migration, and the shell's single-tool-format import branch (so any pre-existing per-tool export file still imports centrally).

**Part 2 — Retire KMQT Board (the tool), keep Reflection Board.** The user confirmed they use *neither* KMQT nor Reflection Board (the latter was a reimagining of the former), but chose to retire only KMQT for now and keep Reflection Board in the sidebar. Full P50-style tool retirement across ~15 files: deleted `kmqt-board.html`; removed the home "Open Questions" dashboard widget, the AI `add_kmqt_item` action (prompt + `_executeAction` + icon/label maps + KMQT context block in `hub-ai.js`), Decision Hub's "Send as KMQT Question" button+fn, Review Hub's "→ KMQT Q" button+fn, the `kmqt-board` entries in `hub-search.js`/`hub-links.js`(resolver+TOOL_NAMES)/`graph-hub.html`(color+label maps)/`index.html` `HEALTH_RESOLVABLE_TOOLS` + `APP_FILE_STORAGE_KEYS`, the `seedKmqt()` starter seed, the `sw.js` precache entry, onboarding-tour KMQT copy, and the tags-hub empty-state text. **Capture Hub's "question" category re-pointed** from KMQT → **Decision Hub** (`routeQuestion` now writes a `decision-hub-v1` record with `type:'question'`, `status:'pending'`).

**Key decisions:**
- **Decision:** Preserve `kmqt_current_v2` data + Reflection Board's "↓ From KMQT Board" import bridge, even though the KMQT *tool* is deleted. **Why:** P50 data-safety rule — never purge user data on a tool retirement; the bridge (reads the raw key) stays the one in-app path to migrate old KMQT notes into Reflection Board if ever wanted. Deletion is reversible via git; silent data loss is not. **Confidence:** high.
- **Decision:** Re-point Capture Hub questions to Decision Hub, not to Reflection Board's Question column. **Why:** Decision Hub is actively used and questions are decision-adjacent; routing into Reflection Board (which the user doesn't use) would send captures into a dead end. **Alternative rejected:** make "question" non-routable (stays in inbox) — leaves captures unrouted. **Confidence:** med.
- **Decision:** Leave harmless passive label-maps that merely *name* kmqt-board (`achievements-hub` activity list, `town-hub` Machi registry, `schedule` source-label map) rather than chase every string. **Why:** they read the preserved data / are display lookups, never present KMQT as a live tool; editing Machi risks the stamped-copy rule for no benefit. Matches P50's "remove tool surfaces, tolerate inert references." **Confidence:** high.
- **Lesson reinforced:** the user tried the same "thinking board" concept twice (KMQT → Reflection Board) and neither stuck, because the *job* (dump-and-sort-into-columns) is already covered by tools they do use (Capture Hub, Journal, Decision Hub). Board-shaped tools keep not sticking for this solo user — weight that against any future "reflection board" proposal.

**Verified:** smoke (auto-discovers pages; PRECACHE 60→59) + flows suites green; targeted checks confirm the Reflection From-KMQT bridge still imports the preserved data, Capture questions land in Decision Hub as `type:'question'`, and `HubLinks.resolveItems('kmqt-board')` returns empty (no live items). Reflection Board still loads clean with its Markdown export intact.

**Files:** `project-hub.html`, `reflection-hub.html`, `index.html`, `hub-ai.js`, `hub-search.js`, `hub-links.js`, `graph-hub.html`, `decision-hub.html`, `review-hub.html`, `capture-hub.html`, `hub-starter-data.js`, `tags-hub.html`, `sw.js`, `CLAUDE.md` · **Deleted:** `kmqt-board.html`
...
### ~~Priority 93 — Enterprise readiness Group C: zero-egress runtime + CSP + organization AI policy~~ ✓ Done `[group: enterprise-readiness]`
Closed the technical security tier while preserving the no-build/static architecture and leaving Workspaces untouched.

- **Self-hosted fonts:** all 30 root HTML pages now load `styles/fonts.css`; 19 browser-optimized WOFF2 subsets cover DM Sans (normal/italic 300–700), Fraunces (normal/italic 300–800 incl. real 800), JetBrains Mono (400–500 with Latin/Greek/Cyrillic/Vietnamese subsets), and Syne (400–800). `vendor/fonts/OFL.txt` + `SOURCES.md` preserve license/source/axes. All font assets are precached; Google Fonts requests and the old runtime font cache path are gone.
- **No runtime AI SDK/CDN:** removed the `esm.sh` dynamic import entirely. `hub-ai.js` now has a small direct Anthropic Messages API client with the same response shape, explicit API version/browser header, structured HTTP errors, and the existing model/payload behavior. This is smaller and has less supply-chain surface than vendoring esm.sh's SDK bundle + Node polyfill chain.
- **Local-only icons:** Stakeholder Map and Tool Portfolio no longer send user-entered domains to Google's favicon service. They render existing initials/emoji locally; URL/domain display remains.
- **Organization AI policy:** new `enterprise-config.js`, loaded immediately before `hub-ai.js` in shell/Focus/Journal. Deployment-owned immutable `aiEnabled:false` is not browser storage and cannot be changed by backup import. It hides every `.ai-surface`, makes `isConfigured()` false, rejects all calls before fetch, and prevents key changes through the app. Network blocking remains defense in depth.
- **Repository CSP:** every root HTML page carries the same tested meta policy: default/script/style/image/font/connect/frame/worker/manifest/object/base/form boundaries; only same-origin runtime resources plus optional `api.anthropic.com` connections. `unsafe-inline` remains explicitly because the existing no-build architecture uses inline scripts/styles and ~1,000 event-handler attributes. Deployment guide retains HTTP-header `frame-ancestors 'self'` because meta CSP cannot enforce it.
- **Offline/service-worker:** `enterprise-config.js`, `styles/fonts.css`, all font assets, OFL, and source record added to `sw.js` PRECACHE (59 → 82 entries); the obsolete external font-cache branch was removed. `enterprise-config.js` gets a special **network-first/no-store** path with cached offline fallback so an online administrator's AI-off change is never delayed by normal stale-while-revalidate behavior.
- **Regression enforcement:** smoke now fails if any page lacks CSP, any root runtime file reintroduces Google Fonts/Google favicon/esm.sh hosts, or any local font asset is absent from PRECACHE. Browser checks intercept the direct Anthropic request and validate model/API headers, then replace the deployment policy with `aiEnabled:false` and prove all marked UI is hidden and zero API requests occur.
- Updated README, SECURITY, PRIVACY, deployment guide, third-party notices, file map, dependency table, script order, and standing rules to the new boundary.

**Key decisions:**
- **Decision:** Remove the Anthropic SDK rather than vendor the esm.sh bundle. **Why:** the app uses only one non-streaming `messages.create` shape; the inspected bundle pulled a Node-process/events/tty polyfill chain. A 35-line direct client preserves behavior while eliminating a runtime dependency and CDN. **Alternative:** vendor the SDK + polyfills — more code and notices for no used capability. **Confidence:** high.
- **Decision:** Keep the deployment AI policy in a static, immutable pre-AI script rather than localStorage. **Why:** user data/imports must never override organization policy; a reviewed deployment file is auditable and centrally host-controlled. **Alternative:** a Settings toggle — user preference, not administrative enforcement. **Confidence:** high.
- **Decision:** Serve the policy network-first while online, with cache only as offline fallback. **Why:** ordinary stale-while-revalidate could execute yesterday's `aiEnabled:true` once after an administrator deployed `false`; a security policy must prefer current server truth. **Confidence:** high.
- **Decision:** Remove remote favicons instead of proxying or caching them. **Why:** domain-derived initials/emoji already exist, preserve recognition, and eliminate disclosure of user-entered domains plus another host. **Confidence:** high.
- **Decision:** Ship a pragmatic CSP with `unsafe-inline`, not pretend the current architecture supports a strict nonce/hash policy. **Why:** removing inline scripts/styles and ~1,000 handler attributes is a separate architectural migration; today's policy still blocks undeclared origins, plugins, base changes, and forms without breaking the app. **Revisit when:** a build/extraction migration is explicitly approved. **Confidence:** high.
- **Decision:** Vendor the full Google-served script subsets (including Latin-ext, Vietnamese, Greek, Cyrillic where provided), not Latin-only. **Why:** enterprise names/content can be multilingual even when Japanese glyphs use system fallbacks; the total offline cost remains modest. **Confidence:** med.

**Verified:** `npm test` green: 82-entry PRECACHE coverage, CSP on all 30 pages, retired egress hosts absent, every page loads without JS/CSP errors, direct Anthropic request contract passes, policy-disabled UI + zero-request assertion passes, and all 13 existing flows pass including the all-28-key backup round trip. Source/static checks confirm no Google Fonts, Google favicon, or esm.sh runtime strings; no new storage keys; `git diff --check` clean.

**Files:** all 30 root HTML pages; `enterprise-config.js` (new), `hub-ai.js`, `theme.css`, `styles/fonts.css` (new), `vendor/fonts/*` (new), `stakeholder-hub.html`, `tool-portfolio.html`, `sw.js`, `tests/smoke.js`, `README.md`, `SECURITY.md`, `PRIVACY.md`, `docs/DEPLOYMENT.md`, `THIRD-PARTY-NOTICES`, `CLAUDE.md`

---

### ~~Priority 94 — Enterprise readiness Group D: versioned release and reviewer handoff~~ ✓ Done `[group: enterprise-readiness]`
Closed the final planned enterprise-readiness group without changing application runtime behavior, storage, or the parked Workspaces direction.

- **Release identity:** `VERSION` is the canonical source from v1.1.0 onward. The historical `v1.0` tag is preserved and explicitly identified as predating the formal contract; current enterprise hardening is recorded as v1.1.0.
- **Auditable change history:** new `CHANGELOG.md` uses dated Semantic Versioning entries, calls out security and network-boundary changes, and links release/compare views.
- **Repeatable publication:** new tag-triggered `.github/workflows/release.yml` proves `tag == v$VERSION`, requires a matching dated changelog entry, requires the tagged commit to descend from `origin/main`, reruns full smoke + interaction flows, creates a `git archive` ZIP, writes its SHA-256 checksum, and publishes both as immutable GitHub Release assets. `.gitattributes` removes development/agent-only surfaces from the distributable archive.
- **Operator contract:** new `docs/RELEASING.md` defines version choice, review gates, annotated tagging, checksum verification, deployment evidence, immutability, support, and rollback. `SECURITY.md` now supports only the latest published release from v1.1.0 onward; `docs/DEPLOYMENT.md` requires a verified release/tag/commit/checksum rather than a moving branch.
- **Reviewer front door:** README now points directly to GitHub Releases, `VERSION`, changelog, release verification, and the completed security/privacy/deployment/notice package. The enterprise roadmap no longer presents release discipline as pending.
- **Regression enforcement:** smoke checks Semantic Versioning syntax, the current changelog entry, and the release workflow's tag validation, full-test, checksum, and publication gates.

**Key decisions:**
- **Decision:** Treat `v1.1.0` as the first release under the formal enterprise contract while preserving `v1.0` as historical. **Why:** rewriting or deleting an existing public tag would destroy provenance; the old tag genuinely existed but lacked today's review/release guarantees. **Confidence:** high.
- **Decision:** Publish a checksummed `git archive` from the reviewed tag rather than a hand-curated deployment folder. **Why:** Git supplies a reproducible commit-bound file set, while `.gitattributes export-ignore` keeps test/agent metadata out without maintaining a fragile second application-file allowlist. **Confidence:** high.
- **Decision:** Use GitHub's bundled `gh` CLI instead of adding a third-party release action. **Why:** it avoids another privileged supply-chain dependency in a workflow with `contents:write`. **Confidence:** high.
- **Decision:** Support only the latest published release, not `main`, historical versions, or an LTS matrix. **Why:** the project has no staffed multi-branch maintenance operation; a narrow honest policy is actionable for both maintainers and enterprise operators. **Revisit when:** maintainers can reliably backport and test multiple release lines. **Confidence:** high.
- **Decision:** Prepare but do not create the v1.1.0 tag from an uncommitted working tree. **Why:** tags must identify an immutable reviewed commit on `main`; the release workflow enforces that boundary after the user commits and pushes. **Confidence:** high.
...
### ~~Priority 104 — Vault Bridge: read the Obsidian vault as a source~~ ✓ Done `[group: vault-bridge]`
Origin: a usage audit of the user's real 2026-08-05 Full Backup, run in an esen-vault session. The numbers were blunt — days the app was opened per month went **31 (May) → 18 (June) → 12 (July) → 1 (August)**, while **24 of 32 recent work-days had an esen-vault daily note but no Thinking Hub session at all**, and the vault held **353 `**Decision:**` blocks against Decision Hub's 19**. Every "process" surface was empty while its "capture" half filled (Meeting Hub: 28 meetings, 20 with notes, **0 action items, 0 decisions**; Learning Log: 38 items, **0 keyInsight**; Time Journal: 1 session ever). The diagnosis was not a missing tool: **the work log moved to the vault**, where notes get written as a side effect of doing the work with an agent at zero marginal cost, while Thinking Hub asks for the same information to be re-typed later. That contest has one outcome.

**The fix is to stop competing for writes and start consuming.** Thinking Hub's Obsidian integration ran one way only — Option A opens a note, Option B indexes titles for autocomplete. Nothing ever flowed vault → hub, which is why three hand-built reconciliation passes (2026-07-26 / 07-30 / 08-05) each produced a reconciled backup that **was never imported**, and why `cdi-proj` went missing twice.

- **`hub-vault-bridge.js`** (new) — `HubVaultBridge`, storage `hub-vault-bridge-v1`.
  - **Persistent directory handle** in IndexedDB (`thinking-hub-vault`). `hub-obsidian.js` only ever held its handle in memory, so every session needed a fresh folder pick — the friction that would have killed this feature. On load `init()` restores the handle and calls `queryPermission`; `granted` scans silently, `prompt` surfaces a one-click **Reconnect** (the browser requires that gesture). New `HubObsidian.adoptHandle()` shares the handle back, so "Re-index" works after a reload too.
  - **Cheap scan (automatic, ~3s after load):** enumerates `daily/**` *filenames only* — no file reads — extracts `YYYY-MM-DD`, and diffs against `_hubActiveDays()`: every ISO timestamp in **all** localStorage (raw string scan, no curated key list — the P81 lesson), anchored on the `T` so plain `YYYY-MM-DD` due/meeting dates are excluded (a task *due* on a date is not evidence anything was recorded that day). Bounded to a 60-day lookback; today is never a gap.
  - **Deep scan (on demand):** reads those notes and parses canonical-schema blocks. Tolerant by necessity — the same vault writes the schema in two shapes: one label per bullet, and several labels inside one wrapped paragraph. Works on absolute offsets, not lines, so a field's text is cleaned once *after* joining.
  - **Review queue** — nothing imports automatically. Each proposal shows Decision / Why / Alternative / Revisit-when / confidence + source path, with per-item Accept (optionally to a project) / Ignore, "Accept all", and per-day Dismiss. Accepted records match `decision-hub.html`'s own `createNew()` shape exactly and carry `obsidianNote` back to the source note.
- **`index.html`** — a `⟡ Vault Bridge` Today card (gap count, or a Reconnect prompt), the review modal (`.ui-modal` + `HubUtils.trapFocus` + Esc), and a Settings → Integrations → Obsidian sub-section (Connect / Rescan / daily-notes folder / status). `styles/index.css` gains the `.vb-*` card styles.

**Key decisions:**
- **Decision:** Invert the Obsidian integration's direction rather than build a new tool. **Why:** the vault is already the richer, cheaper write surface — competing with it loses; consuming it makes every *existing* hub surface true instead of three weeks stale, and finally gives Resurface/Calibration (P51) live data. It adds no sidebar entry. **Alternative rejected:** a new "vault" tool — fails the solo test ([[build-for-solo-not-team]]) and duplicates Obsidian. **Confidence:** high.
- **Decision:** Propose-then-accept; never auto-import. **Why:** a heuristic parser reading prose is exactly the thing that must not write to a decision log unattended — the same discipline the three manual reconciliation passes used. A wrong decision silently added is worse than one missed. **Confidence:** high.
- **Decision:** A bold run counts as a schema label only if it carries a colon **and** maps to a known field alias. **Why:** notes use bold for emphasis mid-sentence (`**Secure Service Account**`); treating every bold run as a boundary truncated field text, and treating only line-start runs as labels missed the wrapped-paragraph shape where `**Why:**` appears mid-line. Colon + known alias separates them cleanly, and unknown colon'd labels (`**Finding:**`) end the current field without closing the block. **Confidence:** high.
- **Decision:** A section heading supplies the title only when it is itself a decision heading (`### Decision — …`); otherwise the title is derived from the decision text and the heading is kept as `context`. **Why:** the first parser gave all three decisions under one `## CDI — …` section the same name. Verified against the real vault. **Confidence:** high.
- **Decision:** Each field's text stops at the first blank line. **Why:** without it the *last* field of a block ran to the next label or EOF and swallowed the note's following prose (real case: 2026-08-03's `Revisit when` absorbed two paragraphs). Continuations inside a field are wrapped/indented without blank lines, so this never truncates real content. **Confidence:** high.
- **Decision:** `hub-vault-bridge-v1` **is** backed up + synced, unlike the other ephemeral UI-state keys. **Why:** its `seen` map is the accept/ignore dedupe — losing it on a restore would re-propose every already-imported decision as a duplicate. That is a data-integrity consequence, not just a stale-UI one. **Confidence:** high.
- **Decision:** Cheap filename-only scan automatic, expensive content scan on demand. **Why:** directory enumeration is fast enough for every load; reading ~1000 notes is 1–3s and belongs behind an explicit click. **Confidence:** high.
- **Deliberately not built:** the third bullet of the original proposal — task/project drift detection. It needs a mapping between vault project notes and hub projects that does not exist (only **1 of 121** tasks has an `obsidianNote`), so it would be name-guessing. Revisit if `obsidianNote` coverage grows.

**Verified** against the **real esen-vault** (`hub-vault-bridge.js` loaded in a VM, parsing the actual 2026-07-27 → 08-05 daily notes): 10 decisions extracted, matching what the 08-05 reconciliation had transcribed by hand, with correct per-field capture and no markdown artifacts. Then a committed suite, `tests/vault-bridge.js` — 27 checks driving the real code path through a stubbed `showDirectoryPicker` backed by fixture notes in both schema shapes: connect, gap detection (incl. the hub-has-a-record day, the >60-day note, and today all correctly excluded), all five schema fields, distinct titles under a shared heading, line-break-spanning bold, non-schema labels, inline multi-label paragraphs, the Today card, the modal, accept/ignore/re-scan dedupe, day dismissal, the IndexedDB round-trip, and silent degradation without the API. Full `smoke` (all 30 pages, CSP, egress, PRECACHE) + `flows` (now 29 backup keys) green.

**Known limitation, stated rather than hidden:** persisting a *genuine* `FileSystemDirectoryHandle` across a reload could not be verified in the sandbox — `showDirectoryPicker` needs a real file dialog, and a fake handle carrying methods is not structured-cloneable (`DataCloneError`, confirmed). The IndexedDB round-trip is asserted directly instead; real handles are serializable per the File System Access spec. **Worth one real check on first use.** Also: on a first connect against a long-neglected vault the queue is large (159 proposals across 39 days on the user's real vault) — that *is* the backlog the feature exists to surface, and the modal scrolls with per-item Ignore and Accept-all, but it is a heavy first sitting.

**Files:** `hub-vault-bridge.js` (new), `tests/vault-bridge.js` (new), `hub-obsidian.js`, `index.html`, `styles/index.css`, `sw.js`, `tests/package.json`, `.github/workflows/smoke.yml`, `PRIVACY.md`, `CHANGELOG.md`, `CLAUDE.md`

---

### ~~Priority 105 — Project Hub reset relocated + archived-items visibility pass + 30-day meeting import~~ ✓ Done `[group: bugfix]`
Three small, independent user-reported fixes.

- **Reset button moved out of per-project Settings** — Project Hub's "⚠ Reset Project Hub" danger-zone card lived inside the *individual project* Settings panel, alongside "Delete Project" — a whole-tool wipe has no business being scoped to one project's settings. Moved to the shell's general ⚙ Settings → Data & Backup pane (its own Danger Zone section, after the backup verifier). The new `resetProjectHubData()` (`index.html`) operates directly on `project-hub-v1` via `HubStorage` — it works whether or not Project Hub is the tool currently open — and refreshes the Project Hub iframe afterward if it is (same reload idiom already used by MCP Sync's "Load from file"). `resetToDefault()` and its per-project button were deleted from `project-hub.html` entirely (dead code, no other call sites).
- **Meeting Hub .ics import window: 7 days back → 30 days back** — `importICSText()`'s `windowStart` and the "no upcoming meetings found" toast text both updated. Import-forward window (90 days) is unchanged.
- **Archived-items visibility pass** — every tool's existing "show archived" toggle (they already existed everywhere; nothing new was built here) got two consistency fixes: (1) label format standardized to `📦 Archived (N)` (previously varied — bare `📦`, `📦 (N)`, icon-only with just a `title` tooltip) across Project Hub (task-level chip), Learning Hub, Meeting Hub, Retro Board, and Risk Hub; (2) the count is colored `var(--accent)` whenever N > 0, so a toggle with waiting archived items is visually distinct from an empty one, not just a static icon easy to overlook. Decision Hub's existing per-project archived badge (only rendered when count > 0, so already reasonably discoverable) had its muted gray recolored to the same accent treatment for consistency. Goals Hub's "Archived Quarters (N)" section was left untouched — it was already the best pattern in the app (a full-width collapsible list with a real "Restore" button per item, not a small icon toggle).
- **Reflection Board bug fix — archived boards were unreachable.** `st.showArchived` existed in state but nothing in the UI ever set it to `true`, so archiving a board removed it from the board-select dropdown with **no path back** — and if it was your only board, `archiveActiveBoard()` silently spawned a brand-new blank "Reflection Board" in its place. Added a working `🗄 Archived (N)` toggle button next to "+ Board" (`toggleShowArchivedBoards()`); `archiveActiveBoard()` now toggles instead of being one-way (mirrors every other tool's archive/restore pattern), and the existing "📦" button relabels itself `↩ Restore` when the active board is already archived.

**Key decisions:**
- **Decision:** Keep `resetToDefault()`'s actual effect (`project-hub-v1` → `{members:[],projects:[],concepts:[]}`) rather than re-seed sample content. **Why:** that's what the original `DEFAULT_STATE` already was — an empty object, not a sample-data seed — despite the confirm dialog's "replaces them with the sample content" wording; preserving the real prior behavior avoids introducing new (and untested) reseed logic for a danger-zone action. Trimmed the now-inaccurate "sample content" phrase from both confirm prompts. **Confidence:** high.
- **Decision:** Don't build a centralized cross-tool archive browser. **Why:** explicit user choice when offered the tradeoff — every tool with archiving already has an in-tool 📦 toggle with Restore; the actual complaint was that these toggles are easy to miss and inconsistent, not that a single browser was missing. **Alternative rejected:** a new "Archived Items" section in ⚙ Data & Backup, or a dedicated sidebar tool — both real options, not chosen. **Confidence:** high (explicit user call).
- **Decision:** Fix Reflection Board's missing toggle as part of this pass rather than filing it separately. **Why:** it's the most severe instance of the exact bug being fixed everywhere else — archived boards weren't just "hard to see," they were **completely unreachable** through the UI, and a board being archived while it's your only one silently fabricates a replacement. Directly in scope. **Confidence:** high.

**Files:** `project-hub.html`, `index.html`, `meetings-hub.html`, `learning-hub.html`, `retro-hub.html`, `risk-hub.html`, `decision-hub.html`, `reflection-hub.html`, `CLAUDE.md`
...
### ~~Priority 111 — Canvas Hub: multiple named boards + typed connections~~ ✓ Done `[group: canvas-structure]`
Second and third options from P110's 5-option brainstorm, both picked by the user in one go: swim-lane Roadmap Mode and multi-board weren't asked for; typed edges (D) and multiple named boards (F) were.

**F — Multiple named boards.** `canvas-v1` now stores `{boards: [{id, name, createdAt, nodes, edges, panX, panY, zoom}], activeBoardId}` instead of one flat `{nodes, edges, panX, panY, zoom}`. `db` — the variable every existing function already reads (`db.nodes`, `db.edges`, `db.panX`, …) — stays a *reference* to the active board object, so the entire rest of the file (drag, resize, pan/zoom, connect mode, PNG export, the P110 kind/link code) needed zero changes; only load/save/switch had to change. `migrateShape(raw)` wraps a legacy flat save into a single board named "Canvas" on first read, so nobody's existing canvas is disturbed. A new top-left board bar (`<select>` + New/Rename/Delete buttons, mirroring Reflection Board's established `.board-select` pattern) switches boards; deleting the last remaining board is refused (`alert`) rather than silently leaving zero boards, matching Reflection Board's own "always at least one" invariant.
- `hub-links.js`'s `resolveItems('canvas-hub')` and `achievements-hub.html`'s "Canvas Painter" node count both read `canvas-v1` directly — both updated to iterate `boards[]` (with a fallback for the pre-migration flat shape, since HubStorage can still legitimately hold a not-yet-touched legacy save), so Cmd+K search and the achievement now correctly aggregate across every board instead of only ever seeing one.
- The `hub-highlight` postMessage handler (fixed for the active-board case in P110) now also searches every board for the target node and calls `switchBoard()` first if it lives elsewhere, so a Cmd+K result on a non-active board actually lands on the right node instead of silently failing to find it.

**D — Typed connections.** Edges gained a `relType` (`relates` / `blocks` / `depends-on` — same vocabulary Dependency Graph's manual links already use, P75) instead of being one undifferentiated dashed line. `relates` stays the original thin dashed accent line; `blocks`/`depends-on` render solid, colored (`--accent-nope` / `--accent2`), with an SVG arrowhead marker showing which way the relationship points. Clicking a connection now opens a small fixed-position popover (type picker + delete) instead of instantly deleting it on click — the old "click = delete" behavior was a single destructive action with no way to express meaning; the popover keeps delete available (a clearly separate red row) while adding the type choice.

**Key decisions:**
- **Decision:** Keep `db` as a live reference into `fullState.boards`, rather than routing every existing node/edge/pan/zoom access through a new `activeBoard()` accessor. **Why:** the file already had ~40 call sites reading `db.nodes`/`db.edges`/`db.panX` etc. before this session even started; reassigning what `db` points to (instead of rewriting every read site) makes board-switching a two-line operation with zero risk to the P110 work just shipped. **Confidence:** high.
- **Decision:** Storage key stays `canvas-v1` — only its internal shape changes. **Why:** `index.html`'s `SCOPE_KEYS.full`/`EXPORT_KEY_LABELS`/`APP_FILE_STORAGE_KEYS` reference storage keys by name; changing the key would need updates across the backup system for zero benefit, and per-board dynamic keys (the alternative considered) would make Full Backup silently incomplete unless every board's key were separately enumerated — the exact P81 bug class this repo has twice already fixed. One key, whole shape, always exported correctly. **Confidence:** high.
- **Decision:** Click-to-delete became click-to-open-a-popover, not click-cycles-type. **Why:** cycling would make "which type is this now" invisible until you'd clicked enough times to notice, and would remove the ability to delete without also cycling through every type first. A popover shows all options plus delete in one glance. **Confidence:** high.
- **Decision:** Deleting the app's last remaining canvas board is refused outright, with no auto-recreate. **Why:** unlike Reflection Board (which auto-creates a fresh blank board when archiving the last one, because *archiving* is non-destructive), deleting a canvas board **destroys its nodes/edges permanently** — silently replacing it with an empty board would hide that the delete already happened. Refusing first ("create another before deleting this one") keeps the destructive action a deliberate two-step, matching this repo's data-safety convention of never silently discarding user content. **Confidence:** high.

**Verified** (Playwright): a seeded legacy flat `canvas-v1` value correctly migrates into one board named "Canvas" with its original node intact; creating a second board via the rename/create prompts, adding a node to it, and switching back to board 1 shows the original board's node untouched and the new board's node invisible on it; `resolveItems('canvas-hub')` (after the save debounce flushes) returns nodes from **both** boards with board-name-prefixed subtitles; deleting the active board switches to the remaining one and deleting the sole remaining board is refused with the expected alert text. For edges: connecting two nodes defaults to `relType:'relates'`; clicking the connection opens the popover showing all three type options; picking "Blocks" sets `relType:'blocks'` and renders with the `arrow-blocks` marker; reopening the popover and choosing delete removes the edge. Zero console errors. Full smoke (30/30 pages) + flows suites green.

**Files:** `canvas-hub.html`, `hub-links.js`, `achievements-hub.html`, `CLAUDE.md`

---

### ~~Priority 112 — Canvas Hub: replace the bulk import modal with a lightweight right-click add + Tool node kind~~ ✓ Done `[group: canvas-structure]`
Closes E from the original 5-option brainstorm — but reshaped per explicit user feedback into something lighter than the plan originally sketched. The user rejected the bulky checklist-modal shape outright: keep free empty-node creation exactly as-is, and add real items one at a time via right-click, "in a lighter and more flexible way... don't wanna make it too complicated."

- **Removed** the "+ From Projects/Goals" toolbar button and its `.ui-modal` checklist overlay entirely (`openImportPicker`/`closeImportPicker`/`confirmImportPicker`, `_importItems`, `_importUntrap`), per the user's explicit choice to replace rather than keep both.
- **New: right-click quick-add menu.** Right-clicking empty canvas opens a small menu: 📝 Empty note, 📌 Project…, 🎯 Goal…, 🛠 Tool…, 🧪 R&D bet. Note/R&D bet create instantly at the click position (same `createNode()` used by double-click, now taking an optional `kind`). The three "…" items open a second-level searchable popover (`add-picker`, live-filtered as you type, auto-focused) listing every real item from that source with its status; picking one drops a live-linked node at the original right-click position via the existing `createLinkedNode()`. Double-click-to-create-a-blank-note is completely unchanged.
- **New node kind: Tool 🛠** (`node.kind === 'tool'`), styled `--node-yellow`/`--border-yellow`, linkable to a real Tool Portfolio entry via the same `node.link = {tool:'tool-portfolio', id, label}` pattern as Goal/Project. `liveToolItems()` reads `tool-portfolio-v1` fresh at render time. `buildKindStrip()` and the action-bar kind toggle were generalized from a project/goal special-case to read from a small `KIND_TOOL_LABEL`/`KIND_PICK_LABEL` map, so adding Tool (and any future kind) doesn't need new branches.
- **Favicon on the Tool node itself** (the user's explicit follow-up ask): the node's kind badge renders the linked tool's real favicon — the identical fetch-with-emoji-fallback pattern Tool Portfolio's own `toolIconHtml()` uses (`https://www.google.com/s2/favicons?domain=<host>&sz=32`, `onerror` → the tool's own emoji) — instead of a generic wrench, so a Tool node reads instantly as "this specific tool," matching the picker row's icon.
- `hub-links.js`'s canonical `resolveItems('canvas-hub')` (used by the shell's own Cmd+K, separate from canvas-hub's in-page override used only when linking from inside the tool) had its `.filter(n => n.text)` widened to also include nodes with no text but a `link.label` (a linked Goal/Project/Tool node with an empty body would previously have been invisible to search).

**Key decisions:**
- **Decision:** Replace the bulk modal outright rather than keep both, per the user's explicit pick when asked. **Why:** two ways to add the same thing (a big checklist vs. one-at-a-time) is exactly the "too complicated" the user was steering away from; the lightweight flow covers the same job at the cost of one extra click per item when adding several at once. **Confidence:** high (explicit user choice).
- **Decision:** Tool Portfolio uses `--node-yellow`/`--border-yellow` for its kind color. **Why:** the only unclaimed node-color/border-color token pair among the six the theme defines — Goal already took the secondary accent, Project took blue, R&D bet took purple(dashed); yellow keeps every kind visually distinct with zero new hex values. **Confidence:** high.
- **Decision:** Real bug found and fixed mid-build — opening the second-level picker from inside the first-level menu's own `mousedown` handler must defer hiding the first menu with `setTimeout(fn, 0)`, not hide it synchronously. **Why:** hiding `#add-menu` synchronously mid-mousedown removes it from hit-testing before the browser's paired `click` event (fired moments later at the same screen coordinates) is dispatched, so that click silently re-targets whatever's underneath (the canvas body) instead of the menu item — which then made the generic outside-click closer treat the click as "outside" and immediately close the picker that had just opened. Deferring the hide by one tick lets the same physical click finish being processed against the still-visible menu first. **Confidence:** high.
- **Decision:** The outside-click closer's guard for `#add-picker` was widened to also treat a click still landing inside `#add-menu` as "not outside" (`!closest('#add-menu') && !closest('#add-picker')`). **Why:** without it, the very click that opens the picker (bubbling up through the menu item that triggered it) would read as an outside-click against the picker and close it on the same interaction — a structurally identical failure mode to the setTimeout fix above, both stemming from one physical click legitimately targeting two different popovers in sequence. **Confidence:** high.

**Verified** (Playwright, seeded a project/goal/tool): the old bulk button is gone; right-click renders all 5 menu items; picking "Tool…" opens the picker showing the seeded tool, live-filters correctly on typing "git", and clicking the row creates a node with the exact expected `{kind:'tool', link:{tool:'tool-portfolio', id, label}}` shape whose kind badge attempts to load `https://www.google.com/s2/favicons?domain=github.com&sz=32`; "Empty note" and "R&D bet" create instantly with no link. Full smoke suite green including two updated favicon-scoping checks (`canvas-hub.html` added as a third opted-in favicon-fetching page alongside Tool Portfolio and Stakeholder Map) + full flows suite green.

**Files:** `canvas-hub.html`, `hub-links.js`, `tests/smoke.js`, `PRIVACY.md`, `CLAUDE.md`
...
### ~~Priority 117 — Bugfix: a THIRD Connect-mode race (kind-strip stopPropagation) + Goal/Project/Tool nodes redesigned as display-only cards~~ ✓ Done `[group: canvas-structure]`
User, right after P116 shipped: "Connect is still not working. There were maybe multiple bugs?" — yes. Also asked to simplify Goal/Project/Tool nodes: no editable "Type here…" text box, just show what they're linked to; Tool nodes especially should lead with the icon (favicon) and the tool's name.

**The third Connect bug.** P113 fixed the plain-note body swallowing clicks; P116 fixed a debounced-save race. Neither covered Goal/Project/Tool nodes, whose `buildKindStrip()` `<select>` and "↗ open" button called `e.stopPropagation()` on `mousedown`/`click` **unconditionally** — including while Connect mode was on. Since that strip covers most of a linked node's clickable area, clicking almost anywhere on a Project/Goal/Tool node never reached the outer node handler where Connect's source/target logic lives, so it looked exactly like "nothing happens" — the same symptom as P113's bug, in a sibling piece of code P113 never touched. Fixed the same way: `stopPropagation()` is now gated on `!connectMode`, so a Connect-mode click bubbles up to the real logic instead of being eaten by the select/button.

**The redesign, which incidentally makes this whole bug class moot for these nodes going forward.** Goal/Project/Tool nodes no longer have an editable text body at all — `node.text` was always `''` for them anyway (nothing ever wrote to it), so the "Type here…" placeholder was dead UI, not a feature. `buildKindStrip()` now renders an icon + the linked item's live name as the node's actual content (`.nk-head`/`.nk-icon`/`.nk-name`); the select-to-relink and "↗ open" row is demoted to `.node.selected .nk-row` — hidden until the node is clicked once, so the resting card just *shows* what it's linked to, exactly as asked, and editing controls appear only when you mean to change them. The small corner "kind badge" (previously duplicating the icon in miniature) is now only rendered for R&D bets, which still have a free-text body and no strip. For Tool nodes specifically the icon is the real favicon at a larger size (24px, up from the old 14px corner badge) so it reads as the dominant visual, per the ask — "icon as main."

**Key decisions:**
- **Decision:** Gate the kind-strip's `stopPropagation()` calls the identical way the note body's already were (`!connectMode`), rather than inventing a different guard. **Why:** it's the exact same hazard in a second location — a descendant unconditionally eating clicks that the ancestor node needs for Connect mode — so the established fix pattern applies verbatim. **Confidence:** high.
- **Decision:** Remove the editable body for these three kinds entirely rather than keep it (empty, hidden, or otherwise). **Why:** `node.text` was never populated for a linked node by any code path — `createLinkedNode()` always sets `text: ''` — so it was pure placeholder chrome with no real use, and removing it is also what directly fixes "only show, don't show a type-here box." **Confidence:** high.
- **Decision:** Hide the select/open row behind `.node.selected`, not behind a separate hover-only or always-visible treatment. **Why:** matches the existing `.node-actions` action-bar precedent in this same file (visible only when selected) — one already-familiar interaction convention, not a second one to learn. **Confidence:** high.

**Verified** (Playwright): seeded already-existing (no pending debounce) Project and Tool nodes; confirmed zero `.node-body` elements exist for them, the icon+name render correctly, and the select/open row is hidden until the node is selected (`display:none` → `flex`). Turned Connect mode on and clicked directly on the `.nk-head` (icon+name) area of each — first click armed the source (`.connect-source` class applied), second click created the edge — reproducing and then fixing the exact "click does nothing" symptom the user reported for these node types specifically. Confirmed the select still works normally to relink an item when the node is selected and Connect mode is off. Full smoke (30/30 pages) + flows + vault-bridge suites green.
...
### ~~Priority 120 — Declutter: delete Reflection Board, Frameworks, Argument Hub~~ ✓ Done `[group: declutter]`
User's own instinct, prompted by a screenshot of Project Hub's mostly-empty Concept Matrix: "I'm thinking to remove the hubs I'm not using at all... by purging little, we might improve to focus on used hubs instead." Reflection Board, Frameworks, and Argument Hub were named as unused; Time Journal was also floated but kept — it's load-bearing for 6 other tools' data (task time badges, Today dashboard, Achievements XP, Journal/Weekly Review, hub-search, hub-links), so removing it would silently blank those out rather than just removing an island. This is the same P50/P88 pattern (delete confirmed-unused tools outright, keep their data) applied to a third round of tools.

**What was deleted:** `reflection-hub.html`, `frameworks-hub.html`, `argument-hub.html`. Removed from every wiring point: `index.html` (APPS entries, Strategy mode's tool list, `APP_FILE_STORAGE_KEYS`, `HEALTH_RESOLVABLE_TOOLS`, AI Context curated key list), `hub-links.js` (`TOOL_NAMES` + both `resolveItems()` branches), `hub-search.js` (Cmd+K `TOOLS`), `hub-tags.js` (Reflection Board's `TAG_SOURCES` entry — matches the P61 precedent of removing a retired tool's tag source outright, not special-casing it), `hub-ai.js` (the Reflection Board rich-context block), `sw.js` (PRECACHE, plus a cache-version bump per the P118 lesson so the removal isn't masked by stale-while-revalidate), `town-hub.html` (`HUB_PAGES` — Machi buildings for these three now render nothing, per the P107 pattern), `journal-hub.html` (the "→ Reflection Q" button and its `rhSendToReflection()` function — a real, working cross-tool bridge into a tool that no longer exists), `help-hub.html` (all three tool cards; framework `tools:` arrays for Scrum/Kaizen/Agile-Retrospectives/KPT-Retrospective/Systems-Thinking/Critical-Path-Method/Pyramid-Principle/SCQA/MECE emptied of the deleted tool per the P50 "keep the framework, drop the dead tool reference" rule; the Weekly Rhythm and Decision Sprint workflow guides had their Reflection-Board/Argument-Hub steps removed, and Idea → Delivery's closing retrospective step was re-pointed to Journal Hub instead of dropped outright, since ending that workflow with no reflection step at all felt like a regression), `README.md`, `ai-upload-guide.md`.

**What was deliberately kept:** `argument-hub-v1`/`reflection-hub-v1` stay in `SCOPE_KEYS.full`, `MCP_SYNC_KEYS`, and `EXPORT_KEY_LABELS` in `index.html` — same data-safety precedent as `scrum-hub-v1`/`hub-warroom-v1` (P50) and `kmqt_current_v2` (P88): code deletion is reversible via git, silent data loss on a backup is not. Removed only from `SCOPE_KEYS.curated` (AI Context — no reason to keep feeding a dead tool's data into AI prompts) and `APP_FILE_STORAGE_KEYS` (the "Current Tool" export scope needs a live file to export from; there isn't one anymore).

**`blocked-depth.html` was left as-is, now more orphaned than before.** It was already unreachable from the sidebar (P107 noted it "never had a top-level `APPS` entry — only reachable as a Frameworks tab"); with `frameworks-hub.html` gone, it has no host at all. Not in scope to fix or delete — the user asked for three specific tools, and this file's own reachability was already a pre-existing, documented condition, not something this change created.

**A real bug found and fixed along the way:** `journal-hub.html`'s "→ Reflection Q" button (`rhSendToReflection()`) actually wrote free-form weekly notes into `reflection-hub-v1` as Question-column cards — a genuine, working integration, not a dead reference like most of what else needed removing. Deleting Reflection Board without also removing this button would have left a working button whose destination silently vanished (worse than the tool being gone outright — the button would still *look* functional).

**Key decisions:**
- **Decision:** Keep Time Journal, delete the other three, per explicit user choice after being shown the tradeoff (Time Journal's 6 downstream consumers vs. the other three being self-contained islands). **Why:** matches this repo's own "solo test" lesson from P50 — a tool nobody opens weekly should go, but a tool that's silently load-bearing for *other* screens the user does use is a different risk profile entirely; removing it would look like fixing clutter but actually blank out data in Project Hub, the home dashboard, and Achievements. **Confidence:** high (explicit user call).
- **Decision:** Repoint framework reference entries (Scrum, Kaizen, Agile Retrospectives, KPT Retrospective, Pyramid Principle, SCQA, MECE, Critical Path Method, Systems Thinking) to an empty `tools:` array rather than deleting the framework entries themselves. **Why:** direct continuation of the P50 decision — these are real, named methodologies with independent reference value in Help & Guide, valid whether or not a tool in this app currently demonstrates them. **Confidence:** high.
- **Decision:** Bump `sw.js`'s `CACHE` constant for this change. **Why:** P118 already established that a batch of file removals/reference changes risks looking "still there" to a user on the first load after deploy under stale-while-revalidate — a deleted tool that still opens (from cache) would be a confusing, avoidable repeat of that exact saga. **Confidence:** high.
- **Decision:** Re-point Idea → Delivery's closing "reflect after delivery" workflow step to Journal Hub instead of removing it outright, while the other two workflows' Reflection-Board steps were removed with no replacement. **Why:** a workflow guide that ends mid-step (triage → explore → decide → build → *nothing*) reads as incomplete in a way the other two workflows' mid-sequence steps don't; Journal Hub is this app's other retrospective surface and was already doing double duty elsewhere in the same guide. **Confidence:** med.

**Verified:** full `npm test` green — `smoke` (27/27 pages now, was 30; sidebar drops from 23 to 20 nav items; no dead egress/CSP/precache regressions), `flows` (Full Backup still restores all 29 keys byte-identical, confirming the orphaned `argument-hub-v1`/`reflection-hub-v1` keys ride along correctly), `vault-bridge` (unaffected, 27/27). A real syntax error (a leftover duplicate `{` from the help-hub.html tool-card deletion) was caught by the smoke suite's per-page JS-error check before being missed — exactly the kind of mechanical slip this suite exists to catch.

**Files:** `index.html`, `hub-links.js`, `hub-search.js`, `hub-tags.js`, `hub-ai.js`, `sw.js`, `town-hub.html`, `journal-hub.html`, `help-hub.html`, `README.md`, `ai-upload-guide.md`, `CLAUDE.md` · **Deleted:** `reflection-hub.html`, `frameworks-hub.html`, `argument-hub.html`

---

### ~~Priority 121 — Project Hub: Concept Matrix transposed to rows=projects + per-project quick-tag drawer~~ ✓ Done `[group: project-ux]`
Follow-up to P120's decluttering pass — the user flagged Concept Matrix itself as unused ("it's all one direction after all"), i.e. purely manual cell-by-cell entry with zero live-data population, which is exactly why every project showed `0` depth in the real screenshot that started this conversation. Considered and rejected reframing it as a live Project × Tool matrix (driven by Tool Portfolio's real "Enabled Projects" links) — the user said tool-overlap isn't the useful axis, the abstract-concept framing is, so the fix targets the actual friction (first-time manual entry) instead of the framing.

**Two changes, requested together:**
- **Axis swap — projects are now rows, concepts are now columns.** Projects will typically outgrow concepts in count, and a tall table scrolls vertically far more comfortably than a wide one scrolls horizontally (the user's own reasoning). `cmSortedConcepts()` was replaced with `cmSortedProjects()` — the existing Sort control ("Most connected"/"Name") now orders projects (rows); concept columns render in a fixed alphabetical order (fewer of them, no need to reorder for scanning). The row-level `∑` badge is now a project's total concept count (previously the footer-only "Project depth" row); the footer is now per-concept "Concept reach →" totals (previously the row-level `∑`). The hover cross-highlight in `attachCmCellEvents()` was flipped from `data-pid` to `data-cid` so hovering a cell now highlights the whole *concept* column (the more useful "where else does this concept show up" comparison, now that concepts are the fewer, narrower axis) instead of the project row, which is already visually distinct as its own `<tr>`.
- **Per-project quick-tag drawer** — a new 🏷 icon on every project row (same slot/style as the old per-concept ✎ edit icon) opens a focused checklist: every concept, each with a None/Related/Core three-button toggle, all in one sitting — instead of hunting individual cells across a wide grid. Prev/Next buttons step through projects in the same sorted order as the grid, so a first-time pass through the whole matrix is N short screens instead of N×M random clicks. Toggling saves immediately (same debounce-free `saveState()` pattern as the grid's own click-to-cycle) and re-renders only the drawer's own list; closing the drawer (`closeProjectTagDrawer()`) re-renders the full grid once to pick up everything changed during the session.

**A real, pre-existing bug found and fixed along the way:** `cm-modal-overlay` (Project Hub's own "New/Edit Concept" modal, unrelated to this session's changes until now) toggled visibility via `element.style.display`, but the shared `.ui-modal-overlay` CSS primitive (`theme.css`) only lifts `opacity:0`/`pointer-events:none` when the element also carries an `.is-open` class — a class this modal's `openConceptModal()`/`closeConceptModal()` never added. The modal has therefore been genuinely unusable by mouse since it shipped: `display:flex` made it occupy layout space, but `opacity:0` kept it invisible and `pointer-events:none` made every button inside it unclickable — only the auto-focused name `<input>` plus Enter-to-submit ever worked. This likely contributed to Concept Matrix looking unused: the CONCEPT_SEEDS quick-add chips (which bypass the modal entirely) were the only way anyone could actually add a concept by mouse. Fixed by switching both `openConceptModal`/`closeConceptModal` to the correct `.is-open` class-toggle pattern already used correctly by the sibling `mx-modal-overlay` (Priority Matrix) — and built the new drawer the same (correct) way from the start, after first reproducing the identical failure by copying the broken pattern and catching it via a real-click Playwright test.

**Key decisions:**
- **Decision:** Swap which axis is rows vs. columns, don't rebuild the interaction model. **Why:** the grid's click-to-cycle semantics, insights panel, and legend were never the complaint — only which axis scrolls how. Reusing the exact same CSS classes (`.cm-proj-th`/`.cm-rotate`/`.cm-label-cell` etc.) for whichever axis they now represent kept the diff to render-function logic only, no new stylesheet surface beyond the drawer itself. **Confidence:** high.
- **Decision:** Sort control now orders projects; concept columns are always alphabetical, with no separate sort control for them. **Why:** matches the user's own framing — projects are the axis expected to grow and need active triage (most-connected-first helps spot which are still empty while scrolling); concepts are few enough that a stable, guessable order (alphabetical) is enough, and a second sort control for the smaller axis would be a UI cost for a rarely-needed capability. **Confidence:** med.
- **Decision:** Fix the pre-existing `cm-modal-overlay` bug in the same pass rather than filing it separately. **Why:** discovered directly while building the drawer (copying its exact — broken — display-toggle pattern first, since it looked like the established local convention), and it's the same modal-visibility plumbing this feature depends on; leaving a second broken modal next to a newly-fixed one in the same file would be an obvious inconsistency. **Confidence:** high.
- **Decision:** Verify with real mouse clicks through Playwright (`locator.click()` on the actual Save/toggle buttons), not just state inspection. **Why:** this exact bug class — an element that occupies layout and *looks* interactable in a static read of the HTML/JS but is actually `pointer-events:none` — is invisible to code review and to a test that only checks `display` or calls functions directly; only a real click reproduces it. This is the same "verification must reproduce the actual failure mode" lesson the Canvas Hub Connect-mode saga (P113–P119) already established for timing races — here it applied to visibility/interactability instead. **Confidence:** high.

**Verified** (Playwright, real mouse clicks throughout — not direct function calls): seeded 3 projects + 2 concepts (one pre-tagged); confirmed rows render as projects in most-connected-first order and columns render as concepts; clicked the "+ Concept" button and the modal's real Save button (previously impossible) and confirmed the new concept persisted; opened the drawer via a project's 🏷 icon, confirmed the correct project title and full opacity/clickability, toggled a concept to "Related" via a real button click and confirmed `{p2:1}` persisted to `project-hub-v1`, used Next to step to a different project, and confirmed closing the drawer refreshed the main grid to show the change. Full `smoke` + `flows` suites green throughout.

**Files:** `project-hub.html`, `styles/project-hub.css`, `CLAUDE.md`
...
8. ~~**Org-level kill-switch for the AI feature.**~~ ✓ `enterprise-config.js` policy + fail-closed HubAI guard + hidden surfaces (P93).
9. ~~**Versioned releases + changelog**~~ ✓ P94: canonical version, changelog, tested tag gate, checksummed immutable release archive, and pin/verify/rollback guidance.
10. ~~**Surface existing strengths in the README**~~ ✓ Final reviewer front door links the complete release/security/privacy/deployment/legal package (P91/P94).
...
A second agent (Codex, reading `AGENTS.md`) built **Machi Hub** (`town-hub.html` + `machi-engine.js` + `machi-achievements.js`) across its own Priorities 79–86, which lived only in AGENTS.md until P81 consolidated docs. Condensed record of the decisions that still bind:
