...
**Files:** `tool-portfolio.html` only — tab bar, `renderMatrix()`, `getFreshnessBucket()`, `categoryPaletteKey()`, popover component (`openChipPopover`/`closeChipPopover`/`logToolUsage`/`setToolMastery`), new fields on `tool_portfolio` storage objects. Per the "category as chip color" decision above (confidence: med, the part of the design the user was least sure of), revisit after 1–2 weeks of real use before adding anything further (filter dropdown, true streak counter).
